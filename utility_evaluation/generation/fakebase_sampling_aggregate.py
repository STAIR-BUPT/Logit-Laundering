#!/usr/bin/env python3
"""
FakeBase Sampling aggregate detector.

Base-only watermark detection with fake-logit swapping followed by
greedy / top-k / top-p token selection, aggregated by book.
"""
...
BASE_MODEL_PATH = os.environ.get("BASE_MODEL_PATH") or os.environ.get("TRAINED_BASE_MODEL", "")

DATASET_PATH = os.environ.get("UTILITY_TEXT_DATASET") or os.environ.get("UTILITY_DATA", "")
TEXT_COLUMN = "text"
SOURCE_FILTER = None
GROUP_BY_FIELD = "id"
OUTPUT_JSON = str(MODULE_ROOT / "outputs" / "fakebase_sampling_aggregate.json")
NUM_SAMPLES = 0

FAKE_PROB = 0.3
FAKE_K = 5
FAKE_MODE = "semantic_topk"
...
def compute_group_stats(
    ...
    fake_trigger_rate = fake_trigger_count / total_steps if total_steps > 0 else 0.0
    return {
        ...
        "fake_trigger_count": int(fake_trigger_count),
        "fake_trigger_rate": float(fake_trigger_rate),
...
def print_group_result(result: dict, decode_spec: str, prefix: str = "") -> None:
    ...
    print(
        ...
        f'fake_rate={result["fake_trigger_rate"]:.4f} '
        f"{status}"
    )
...
class FakeSwapper:
    def __init__(self, base_model, fake_prob: float, fake_k: int, fake_mode: str) -> None:
...
def count_green_hits_with_sampling(
    ...
    if input_ids.size(1) <= K:
        return stats, n_steps, fake_triggers
    ...
    for i in range(K, input_ids.size(1) - 1):
        ...
        fake_row = base_last.clone()
        if fake_swapper.maybe_swap(fake_row):
            fake_triggers += 1
        n_steps += 1
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="FakeBase Sampling aggregate detector")
    ...
    parser.add_argument("--fake_prob", type=float, default=FAKE_PROB)
    parser.add_argument("--fake_k", type=int, default=FAKE_K)
    parser.add_argument(
        "--fake_mode",
        type=str,
        default=FAKE_MODE,
        choices=["top2", "synonym", "semantic_topk"],
    )
    return parser
...
def main():
    ...
    print("FakeBase Sampling aggregate detection")
    print(f"   base_model_path={args.base_model_path}")
    print(f"   seeding_scheme={args.seeding_scheme}")
    print(f"   fake_prob={args.fake_prob}  fake_k={args.fake_k}  fake_mode={args.fake_mode}")
    ...
    print(f"   end_group_index={'ALL' if args.end_group_index <= 0 else args.end_group_index}")
    print("=" * 60)
    ...
    print(f"Group count: {len(grouped_records)}")
    print("=" * 60)

    model, tokenizer = load_model(args.base_model_path)
    wm = load_watermark_detector(tokenizer, args.base_model_path, args.seeding_scheme)
    fake_swapper = FakeSwapper(
        base_model=model,
        fake_prob=args.fake_prob,
        fake_k=args.fake_k,
    ...
    if not selected_group_items:
        raise ValueError("selected group range is empty")
    ...
    for idx, (group_name, group_items) in enumerate(
        ...
        for item in tqdm(
            ...
            per_text_stats, per_text_steps, per_text_fake_triggers = count_green_hits_with_sampling(
                model=model,
                tokenizer=tokenizer,
                wm=wm,
                fake_swapper=fake_swapper,
                text=item[args.text_column],
                decode_specs=decode_specs,
                generators=generators,
            )
            total_steps += per_text_steps
            total_fake_triggers += per_text_fake_triggers
            for decode_cfg in decode_specs:
                decode_spec = decode_cfg["decode_spec"]
        ...
        for decode_cfg in decode_specs:
            ...
            result = compute_group_stats(
                ...
                fake_trigger_count=total_fake_triggers,
                gamma=GAMMA,
            )
