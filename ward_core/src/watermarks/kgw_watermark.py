...
class KgwWatermark(BaseWatermark):
    ...
    def get_prevctx_width(seeding_scheme: str) -> int:
        if seeding_scheme == "selfhash":
            return 3
        elif seeding_scheme == "lefthash" or seeding_scheme == "simple_1":
            return 1
    ...
    def spawn_logits_processor(self, strength=None) -> WatermarkLogitsProcessor:
        if strength is None:
            strength = self.cfg.generation.delta
