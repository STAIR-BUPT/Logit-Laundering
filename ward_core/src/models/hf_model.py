...
class HfModel:
    ...
    #     if 'Llama-3.1-70B' in self.cfg.name: 
    ...
    #             self.cfg.name, quantization_config=quantization_config,
    #             use_flash_attention_2=self.cfg.use_flashattn2,
    #             device_map="cuda:0",
    #         )
    #     elif "Llama-3" in self.cfg.name:  # ✅ Added Llama-3 support
    #         self.model = AutoModelForCausalLM.from_pretrained(
    #             self.cfg.name,
    ...
    #     if self.device == "cuda" and 'Llama-3.1-70B' not in self.cfg.name:
    #         self.model.to("cuda")
    #     self.model.eval()
    ...
    def _load_model(self) -> None:
        ...
        # === Helper: only pass kwargs that are supported by from_pretrained ===
        def safe_kwargs(model_cls, base_kwargs: dict, **extra_kwargs):
        ...
        def load_causal_lm_with_fallback(name: str, kwargs: dict):
            """
            Try loading as CausalLM first; if transformers explicitly rejects the config for
            AutoModelForCausalLM, fall back to a multimodal ConditionalGeneration AutoModel
            (e.g. mistral3) so that plain-text logits can still be obtained.
            """
            try:
                return AutoModelForCausalLM.from_pretrained(name, **kwargs)
            except ValueError as e:
                msg = str(e)
                if "Unrecognized configuration class" in msg and "AutoModelForCausalLM" in msg:
                    # Re-inject safe kwargs for the fallback AutoModel (signature may differ slightly)
                    use_fa2 = getattr(self.cfg, "use_flashattn2", False)

                    # 1) Try ImageTextToText first (mistral3/pixtral usually map here)
                    kwargs2 = dict(kwargs)
                    if use_fa2:
                        kwargs2 = safe_kwargs(
                            AutoModelForImageTextToText,
                    ...
                    try:
                        return AutoModelForImageTextToText.from_pretrained(name, **kwargs2)
                    except Exception:
                        pass

                    # 2) Fall back to Vision2Seq (covers another set of multimodal generation models)
                    kwargs3 = dict(kwargs)
                    if use_fa2:
                        kwargs3 = safe_kwargs(
                            AutoModelForVision2Seq,
                    ...
                    return AutoModelForVision2Seq.from_pretrained(name, **kwargs3)
                raise
        ...
        if "Llama-3.1-70B" in self.cfg.name:
            ...
            kwargs = dict(
                ...
                device_map="cuda:0",
                torch_dtype=getattr(self.cfg, "torch_dtype", torch.float16),
            )

            # Add flash-attention kwargs if supported by transformers
            use_fa2 = getattr(self.cfg, "use_flashattn2", False)
            if use_fa2:
                kwargs = safe_kwargs(AutoModelForCausalLM, kwargs, use_flash_attention_2=True, attn_implementation="flash_attention_2")

            # 70B quantized: load as CausalLM, no fallback
            self.model = AutoModelForCausalLM.from_pretrained(self.cfg.name, **kwargs)

        # === Llama-3.x / Llama-2 ===
        elif "Llama-3" in self.cfg.name or "Llama-2" in self.cfg.name:
            kwargs = dict(
                torch_dtype=torch.float16 if getattr(self.cfg, "use_fp16", True) else torch.float32,
                device_map=getattr(self.cfg, "device_map", None),
            )
        ...
        # === Pegasus ===
        elif "pegasus" in self.cfg.name.lower():
            self.model = PegasusForConditionalGeneration.from_pretrained(self.cfg.name)
        ...
        # === Generic decoder-only (GPT/Llama/OPT etc.) ===
        elif is_decoder_only_model(self.cfg.name):
            kwargs = dict(
                torch_dtype=torch.float16 if getattr(self.cfg, "use_fp16", True) else torch.float32,
                device_map=getattr(self.cfg, "device_map", None),
            ...
            # ✅ Support mistral3-style “ConditionalGeneration” architectures: fall back when CausalLM is rejected
            self.model = load_causal_lm_with_fallback(self.cfg.name, kwargs)

        # === Dipper (T5-family) ===
        elif "dipper" in self.cfg.name.lower():
            self.model = T5ForConditionalGeneration.from_pretrained(self.cfg.name)
        ...
        # === Device placement ===
        if torch.cuda.is_available() and "Llama-3.1-70B" not in self.cfg.name:
            # Skip .to() if model already uses device_map (automatic device placement)
            has_device_map = hasattr(self.model, "hf_device_map") and self.model.hf_device_map is not None
            if not has_device_map:
                # Use self.device rather than hardcoded "cuda"
    ...
    # TODO enable sysprompt for some
    def _encode_batch(
        self,
        prompts: List[str],
    ...
    def generate(
        ...
        if logit_processors is not None and len(logit_processors) > 0:
            print("Using logit processors!!!")
