...
class SynonymAttack(Attack):
    ...
    def warp(self, text):
        ...
        rslt = self.warp_sentence(text[:separators[0]])
        for i in range(len(separators)-1):
            rslt +=  self.warp_sentence(text[separators[i]:separators[i+1]])
        return rslt
    ...
    def wordNetSynonyms(self, word, pos):
        ...
        Returns all lemmas in each synset (previously only the first was returned),
        giving a larger set of synonym options.
        """
        all_lemmas = []
        for s in wordnet.synsets(word, pos=pos):
            # Collect all lemmas (synonyms) in this synset
            all_lemmas.extend([l.name() for l in s.lemmas()])
        # Deduplicate and return
        return list(set(all_lemmas))
    ...
    def warp_sentence(self, sentence):
