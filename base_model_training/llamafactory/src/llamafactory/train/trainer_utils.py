...
def format_metrics_for_log(metrics: Mapping[str, Any]) -> dict[str, Any]:
    """
    将 metrics 规整为 `Trainer.log_metrics()` 可安全打印的形式。

    - dict: 跳过（log_metrics 不支持嵌套 dict）
    - torch.Tensor/numpy 标量: 转为 python 标量
    - list/tuple: 若是数值序列则取均值/单元素；否则转为字符串
    - 其他对象: 尝试转 float，否则转 str
    """
    ...
    def _to_scalar(v: Any) -> Any:
        ...
        if isinstance(v, dict):
            return None  # handled by caller (skip)
        if isinstance(v, (str, bool, int, float)):
            return v
        if isinstance(v, torch.Tensor):
            if v.numel() == 1:
                return v.detach().cpu().item()
            return v.detach().float().mean().cpu().item()

        # numpy types/arrays（避免硬依赖 numpy）
        if hasattr(v, "dtype") and hasattr(v, "shape") and hasattr(v, "item"):
            try:
                return v.item()
            except Exception:
                pass
        if hasattr(v, "mean") and hasattr(v, "astype"):
            try:
                m = v.mean()
                if hasattr(m, "item"):
                    return m.item()
